    ...
    <div class="w-full">
        <div class="sticky top-[62px]"
            ...
            <div class="bg-body pt-4">
                <PostReplyForm :post-id="postId"
                               @new-reply="addReply" />
            </div>
        </div>
        <div class="mt-4 flex flex-col gap-4">
            <template v-if="!isBusy">
                <div v-for="postReply in postReplies"
                     :key="postReply.id">
                    <PostReplySingle v-if="postReplyIdToEdit !== postReply.id"
                                     :post-reply="postReply"
                                     @update="update(postReply.id, $event)"
                                     @edit-post-reply="postReplyIdToEdit = $event"
                                     @delete-post-reply="postReplyIdToDelete = $event" />
                    <PostReplyEdit v-else
                                   :post-reply="postReply"
                                   @cancel="postReplyIdToEdit = null"
                                   @confirm="editPostReply" />
                </div>
            </template>
            <template v-else>
                <PostReplySingleLoader v-for="i in 3"
                                       :key="i" />
            </template>
        </div>
        <ConfirmPopup v-if="postReplyIdToDelete"
                      @close="postReplyIdToDelete = null"
                      @confirm="deletePost">
