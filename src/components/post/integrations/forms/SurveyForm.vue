<template>
    <div class="relative border border-zinc-300 rounded-md px-4 pt-4">
        <div class="absolute top-0 left-4 text-zinc-300 text-sm font-semibold -translate-y-1/2 bg-secondary px-1 uppercase">
            Sondage
        </div>
        <div class="flex">
            <text-input v-model="data.question"
                        placeholder="Question"
                        no-margin
                        @change="$emit('update', data)" />
            <div class="w-16 pl-4 flex justify-center items-center">
                <button class="text-zinc-300 hover:text-rose-500 font-semibold"
                        @click="$emit('delete')">
        ...
        <div class="w-2/3">
            <div v-for="(option, index) in data.options"
                ...
                <text-input v-model="data.options[index]"
                            input-class="rounded-l-none"
                            placeholder="Nouveau choix"
                            no-margin
                            @change="$emit('update', data)" />
                <cancel-button class="h-8"
                               @click="removeOption(index)">
                    <font-awesome-icon icon="fa-solid fa-xmark" />
                </cancel-button>
            </div>
            <div class="flex gap-4 items-center">
                <text-input v-model="newOption"
                            @enter="addOption" />
                <div>
