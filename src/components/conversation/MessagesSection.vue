<template>
    <div class="bg-body border border-selected rounded-md text-color" style="height: 85vh;">
        ...
        <div class="px-2 overflow-y-scroll flex flex-col-reverse pb-2" style="height: calc(85vh - 114px)">
            <template v-if="!isBusy || !conversation.lastMessage">
                <div v-for="(message, i) in messages" :key="message.id">
                    <div v-if="i === messages.length - 1 || (i <= messages.length - 1 && formatDate(message.createdAt) !== formatDate(messages[i + 1].createdAt))"
                        ...
                        <div class="w-full h-px border-b border-muted"></div>
                ...
                <div v-if="!messages.length || (messages.length && messages[messages.length - 1].id === messageBoundary.first)" class="text-muted px-8 py-4">
        ...
        <div class="h-16 flex items-center gap-4 px-4 bg-secondary border-t border-selected rounded-b-md">
            <textarea v-model="newMessage" rows="1" class="w-full resize-none flex border rounded-md shadow hover:outline-none outline-none border-secondary bg-body focus:border-zinc-600 p-1" @keydown.enter="sendMessage($event)"></textarea>
