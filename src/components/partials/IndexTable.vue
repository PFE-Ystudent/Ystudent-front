                        ...
                        <tr v-for="row in data" :key="row.id">
                            <td v-for="line in headers"
                                ...
                                <template v-if="line.type && line.type === 'bool'">
                                    <input type="checkbox" disabled :checked="row[line.key] ? 1 : 0" />
                                </template>
                            ...
                            <td>
                                <div class="flex justify-end gap-2">
                                    <submit-button icon="fa-edit" @click="$emit('edit', row)" />
                                    <submit-button v-if="!row.isArchived" icon="fa-trash" @click="deleteRow(row.id)" />
                                    <submit-button v-else icon="fa-trash-can-arrow-up" @click="restoreRow(row.id)" />
                                </div>
                            </td>
                        </tr>
                    </template>
                    <tr v-else>
                        <td :colspan="headers.length" class="text-center">Aucune donnée</td>
                    </tr>
                </template>
                <tr v-else>
